<?php
session_start();

include '../../../assets/config/dbconfig.php';

if (isset($_SESSION['admin'])) {
    $adminEmail = $_SESSION['admin'];
} else {
    header("location: ../../attendance");
    exit();
}

if (empty($_POST['date'])) {
    $msg = "Date Required";
    header("location: ../../edit-attendance?err=$msg");
    exit();
}

$date = $_POST['date'];

$attendance = isset($_POST['attendance']) ? $_POST['attendance'] : "";



$sql = "select * from employee";
$results = mysqli_query($con, $sql);
$rescheck = mysqli_num_rows($results);

if ($rescheck > 0) {
    while ($row = mysqli_fetch_assoc($results)) {
        $id = $row['empId'];
        if (isset($attendance[$id]) && $attendance[$id] == 'Present') {
            $sqlAtt = "UPDATE attendance SET status = 'Present' WHERE date = '$date' AND empId = '$id'";
            $result = mysqli_query($con, $sqlAtt);
        } else {
            $sqlAtt = "UPDATE attendance SET status = 'Absent' WHERE date = '$date' AND empId = '$id'";
            $result = mysqli_query($con, $sqlAtt);
        }
    }
}

if ($result) {
    $msg = "Attendance Added";
    header("location: ../../attendance?suc=$msg");
}
